...
package UnitTests
  ...
  model Validate "Assert correctness of a whole solution trajectory"
    ...
    Base.AssertContinuousTrajectory check(MaxAccErr = MaxAccErr, name = name);
  ...
  annotation (
    Documentation(info = "<html><head></head><body>This package contains unit tests for validating some of the components.<div><br></div><div>Developer: Atiyah Elsheikh</div></body></html>"));
end UnitTests;
